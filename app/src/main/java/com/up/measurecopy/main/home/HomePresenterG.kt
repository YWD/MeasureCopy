 ...
 * 首页数据请求类
 */
class HomePresenterG<T>(val context: Context) {
    ...
    fun getHomeConfigG(): HomePresenterG<T> {
        val homeService: HomeService = retrofit.create(HomeService::class.java)

        val timestamp = System.currentTimeMillis() / 1000
        val paramsMap = mutableMapOf<String, String>(Pair("timestamp", "" + timestamp))
        ...
        val homeConfig = homeService.getHomeConfigG<T>(timestamp, sign)
        homeConfig.observeOn(AndroidSchedulers.mainThread()).subscribeOn(Schedulers.io()).subscribe(object : Observer<Response<T>> {
